"""Token-level Gaussian-noise intervention runner (no-pair variant).

For each experiment entry built from traces/pairs:
1. Run the clean prompt forward pass and score logprob at truncation target.
2. For every token position before truncation, replace that input token embedding
   with Gaussian noise (multiple trials, averaged).
3. Measure delta logprob of the true next token at truncation.
...
input-token embedding interventions only.
"""
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run token-level noisy embedding interventions and export matrices.")
    parser.add_argument("--input-json", type=Path, default=INPUT_JSON, help="Input JSON with top-level 'pairs', 'traces', or 'experiments'.")
    ...
    parser.add_argument("--experiment-ids", type=str, default=None, help="Comma-separated experiment IDs. Numeric values also act as index/pair fallbacks.")
    ...
    parser.add_argument("--noise-mode", type=str, default=NOISE_MODE, choices=["gaussian", "zero"], help="Embedding replacement mode.")
    parser.add_argument("--noise-scale", type=float, default=NOISE_SCALE, help="Scale factor applied to Gaussian noise.")
    parser.add_argument("--noise-seed", type=int, default=NOISE_SEED, help="Base seed for deterministic per-token noise.")
    parser.add_argument(
        "--noise-samples-per-token",
        "--noise-samples-per-cell",
        dest="noise_samples_per_token",
        type=int,
        default=NOISE_SAMPLES_PER_TOKEN,
        help="Number of noisy interventions to average for each token position.",
    )
...
def make_noise_embedding(reference_embed: torch.Tensor, experiment_id: str, token_pos: int, sample_idx: int) -> torch.Tensor:
    if NOISE_MODE == "zero":
        return torch.zeros_like(reference_embed)

    ref = reference_embed.float()
    ref_std = float(ref.std(unbiased=False).item())
    if not np.isfinite(ref_std) or ref_std <= 0:
        ref_std = 1.0

    seed = stable_int_seed(NOISE_SEED, experiment_id, token_pos, sample_idx)
    generator = torch.Generator(device=reference_embed.device)
    generator.manual_seed(seed)
    noise = torch.randn(reference_embed.shape, device=reference_embed.device, dtype=reference_embed.dtype, generator=generator)
    return noise * (ref_std * float(NOISE_SCALE))


def forward_logits_batch_with_noisy_tokens(
    model,
    ...
    """Batch forward where each item has one noisy token embedding position."""
    batch_size = len(token_positions)
    if batch_size == 0:
        raise ValueError("token_positions must be non-empty")

    input_ids = torch.tensor([target_token_ids for _ in range(batch_size)], device=DEVICE, dtype=torch.long)
    ...
    with torch.inference_mode():
        ...
        for batch_idx, token_pos in enumerate(token_positions):
            if 0 <= token_pos < seq_len:
                reference = inputs_embeds[batch_idx, token_pos, :]
                inputs_embeds[batch_idx, token_pos, :] = make_noise_embedding(reference, experiment_id, token_pos, sample_idx)
...
def main():
    ...
    global NOISE_SCALE
    global NOISE_SEED
    global NOISE_SAMPLES_PER_TOKEN
    ...
    NOISE_SAMPLES_PER_TOKEN = args.noise_samples_per_token

    print("\n" + "=" * 100)
    print("TOKEN-LEVEL NOISE PATCHING ENGINE")
    print("=" * 100)
    ...
    print(f"  Noise samples/token: {NOISE_SAMPLES_PER_TOKEN}")
    ...
    for exp_idx in exp_indices:
        ...
        print("    Phase 1: baseline clean forward and true-next-token logprob")
        ...
        print("    Phase 2: batched multi-sample token embedding noise")
        ...
        for batch_start in range(0, len(pos_indices), PATCH_BATCH_SIZE):
            ...
            for sample_idx in range(NOISE_SAMPLES_PER_TOKEN):
                patched_logits = forward_logits_batch_with_noisy_tokens(
                    model=model,
            ...
            for pos in batch_positions:
                ...
                noise_delta_matrix[0, col] = float(np.mean(trial_array))
                noise_delta_std_matrix[0, col] = float(np.std(trial_array, ddof=0))
        ...
        matrix_payload = {
            ...
            "intervention_mode": "token_embedding_noise",
            "noise_mode": NOISE_MODE,
            "noise_scale": NOISE_SCALE,
            "noise_seed": NOISE_SEED,
            "noise_samples_per_token": NOISE_SAMPLES_PER_TOKEN,
